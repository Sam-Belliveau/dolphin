...
float4 AreaSampling(float3 uvw, float gamma)
{
    ...
    avg_color += area_nw * QuickSample(float2(f_beg.x + offset, f_beg.y + offset), uvw.z, gamma);
    avg_color += area_ne * QuickSample(float2(f_end.x + offset, f_beg.y + offset), uvw.z, gamma);
    avg_color += area_sw * QuickSample(float2(f_beg.x + offset, f_end.y + offset), uvw.z, gamma);
    avg_color += area_se * QuickSample(float2(f_end.x + offset, f_end.y + offset), uvw.z, gamma);
    ...
    for (int ix = 0; ix < max_iterations; ++ix) {
        if (ix < x_range) {
            ...
            avg_color += area_n * QuickSample(float2(x + offset, f_beg.y + offset), uvw.z, gamma);
            avg_color += area_s * QuickSample(float2(x + offset, f_end.y + offset), uvw.z, gamma);
        }
    }
    ...
    for (int iy = 0; iy < max_iterations; ++iy) {
        if (iy < y_range) {
            ...
            avg_color += area_w * QuickSample(float2(f_beg.x + offset, y + offset), uvw.z, gamma);
            avg_color += area_e * QuickSample(float2(f_end.x + offset, y + offset), uvw.z, gamma);

            for (int ix = 0; ix < max_iterations; ++ix) {
                if (ix < x_range) {
                    float x = f_beg.x + 1.0 + float(ix);
                    avg_color += QuickSample(float2(x + offset, y + offset), uvw.z, gamma);
                }
            }
        }
    }
